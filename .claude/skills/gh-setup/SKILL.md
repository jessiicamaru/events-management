...
`github.com/jessiicamaru/events-management`. There are two, and **either one is enough**.
...
| `--repo <owner/name>` | Target a different repo than `jessiicamaru/events-management`. |
...
```bash
gh repo view jessiicamaru/events-management --json name,defaultBranchRef
gh pr list --limit 3
...
`events-management` repository, with these **repository permissions**:
